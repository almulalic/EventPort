...
public class EventController {
  ...
  public ResponseEntity<List<EventViewDTO>> getUserLikedEvents(
    HttpServletRequest request,
    Pageable pageable
  ) {
    User user = ((User) request.getAttribute("user"));
    return ResponseEntity.ok(eventService.getUserLikedEvents(user.getId(), pageable).stream()
                               .map(EventViewDTO::new)
                               .toList());
  }
  ...
  public ResponseEntity<List<EventViewDTO>> getUserCreatedEvents(
    HttpServletRequest request,
    Pageable pageable
  ) {
    User user = ((User) request.getAttribute("user"));
    return ResponseEntity.ok(eventService.getUserCreatedEvents(user.getId(), pageable)
                               .stream()
                               .map(EventViewDTO::new)
                               .toList());
  }
